...
\begin{table}[H]
\centering
\caption{Eficiencia computacional. Latencia mediana sobre RTX 3060 Mobile, \textit{batch}~1, clip $T=64$. Se incluye X3D-M como referencia 3D eficiente adicional.}
...
MobileNetV2 plano (E7) & 2.2  & 19.8          & 49.6           \\
MobileNetV3     & 3.0          & \textbf{14.3} & \textbf{39.9}  \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Frontera Pareto en AQA-7. SRCC reportado con semilla 42 (\textit{Students}: media de 3 semillas donde se indica). FLOPs y latencia tomados de la Tabla~\ref{tab:eficiencia}.}
\label{tab:pareto}
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
SlowFast-R50    & 33.6  & $\sim$250 & ---   & 0.9158 \\
I3D-R50         & 27.2  & 228.3     & 133.9 & 0.9052 \\
\midrule
X3D-M           & 2.0   & 35.4      & 74.8  & 0.8945 \\
TSM-MobileNetV2 & 2.2   & 20.0      & 54.3  & 0.9021 \\
MobileNetV2 plano (E7) & 2.2 & 19.8 & 49.6 & 0.8921 \\
MobileNetV3     & 3.0   & 14.3      & 39.9  & 0.8907 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item TSM-MobileNetV2 \emph{domina} la frontera Pareto en el rango de costo bajo: alcanza \ac{SRCC} 0.9021 con 20~GFLOPs, mejor que X3D-M (0.8945 con 35.4~GFLOPs) y mejor que MobileNetV3 (0.8907 con 14.3~GFLOPs) en términos absolutos.
    \item MobileNetV3 domina la frontera de costo mínimo: el menor \ac{FLOPs} y latencia, con sólo $\sim 1$ punto de pérdida en \ac{SRCC}.
    \item X3D-M, pese a usar convoluciones 3D explícitas, no supera al \textit{pipeline} 2D+TSM con la misma receta, lo que sugiere que para AQA en arquitecturas livianas el modelado temporal por \textit{shift} es competitivo frente a 3D denso con un tercio de los \ac{FLOPs}.
\end{itemize}

La conclusión práctica es que el \textit{pipeline} propuesto ocupa la frontera Pareto en el régimen de bajo costo, y cualquier extensión 3D liviana posterior (X3D, MoViNet) debería competir contra esta frontera en lugar de contra el Teacher pesado.
...
\begin{table}[H]
\centering
\caption{Comparación con el estado del arte en AQA-7 y MTL-AQA. Los métodos previos usan mayoritariamente \ac{I3D} como \textit{backbone} (28--250~GFLOPs); los métodos propuestos usan arquitecturas livianas (14--20~GFLOPs).}
\label{tab:sota}
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
\textbf{MobileNetV3 (\textit{pipeline})}     & 2026 & 0.891          & 0.870 & \textbf{14.3} \\
\textbf{TSM-MobileNetV2 (\textit{pipeline})} & 2026 & \textbf{0.902} & 0.880 & 20.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item En \textbf{AQA-7}, el \textit{pipeline} propuesto (0.902) supera a varios métodos SOTA basados en \ac{I3D} (USDL, GAKD, CoRe, TSA-Net, HGCN) usando aproximadamente el $9\%$ de sus \ac{FLOPs}. Este resultado refuerza la tesis: una receta moderna eleva el techo del \textit{Student} liviano a un nivel donde los métodos basados en \textit{backbones} pesados con recetas anteriores pierden su ventaja.
    \item En \textbf{MTL-AQA}, el \textit{pipeline} queda aproximadamente 7--8 puntos por debajo del SOTA (TPT, CoRe). En este dataset especializado de clavados, los métodos SOTA incorporan componentes específicos (estructura de \textit{Temporal Parser}, regresión contrastiva, atención por grupos) que aportan en ese régimen pero requieren \textit{backbones} 3D pesados y entrenamiento más costoso.
...
El aporte del trabajo se ubica en una zona específica del espacio de soluciones: \emph{precisión competitiva con el SOTA en al menos un dominio (AQA-7), a costo computacional radicalmente menor, y con resultados estables entre semillas}.
...
AQA-7     & MTL-AQA  & MBv3      & 0.7150  & 0.7320  & $+0.017$ \\
AQA-7     & JIGSAWS  & TSM-MBv2  & $-0.193$ & $-0.084$ & $+0.109$ \\
AQA-7     & JIGSAWS  & MBv3      & 0.037   & 0.142   & $+0.105$ \\
\midrule
MTL-AQA   & AQA-7    & TSM-MBv2  & 0.5294  & 0.5536  & $+0.024$ \\
MTL-AQA   & AQA-7    & MBv3      & 0.5531  & 0.5728  & $+0.020$ \\
MTL-AQA   & JIGSAWS  & TSM-MBv2  & 0.142   & 0.198   & $+0.056$ \\
MTL-AQA   & JIGSAWS  & MBv3      & 0.215   & 0.247   & $+0.032$ \\
\midrule
JIGSAWS   & AQA-7    & TSM-MBv2  & $-0.058$ & 0.061  & $+0.119$ \\
JIGSAWS   & AQA-7    & MBv3      & 0.027   & 0.103   & $+0.076$ \\
JIGSAWS   & MTL-AQA  & TSM-MBv2  & 0.094   & 0.151   & $+0.057$ \\
JIGSAWS   & MTL-AQA  & MBv3      & 0.118   & 0.165   & $+0.047$ \\
\bottomrule
\end{tabular}
\end{table}

La recalibración de \textit{BatchNorm} mejora 11 de las 12 configuraciones evaluadas, con ganancias particularmente notables en los pares de mayor \textit{domain shift} (AQA-7 $\to$ JIGSAWS y JIGSAWS $\to$ AQA-7, donde la transferencia \textit{zero-shot} colapsaba a valores cercanos a cero o negativos). En el único caso donde TTA degrada (AQA-7 $\to$ MTL-AQA, TSM-MBv2), la pérdida es marginal ($-0.009$) y compatible con ruido de estimación.

Este resultado constituye, según nuestra revisión de literatura, la primera aplicación documentada de \textit{Test-Time Adaptation} a \ac{AQA}. La técnica no introduce costo adicional de entrenamiento ni requiere etiquetas del dominio \textit{target}; el único costo es una pasada hacia adelante sobre clips no etiquetados durante la fase de despliegue.
