...
\begin{table}[H]
\centering
\caption{Resultados de precisión en MTL-AQA y JIGSAWS (semilla 42). Se incluyen los regímenes \textit{Pipeline liviano} (sólo $\mathcal{L}_{reg}$) y \textit{+~KD} (análisis marginal). X3D-M se incluye como \textit{Student} 3D liviano del \textit{pipeline}.}
...
\midrule
I3D             & Teacher          & MTL-AQA & 0.8869          & 0.8848          & \textbf{5.51}  \\
\textbf{X3D-M}  & Pipeline liviano & MTL-AQA & \textbf{0.8937} & ---             & ---            \\
...
\textbf{X3D-M (\textit{pipeline})}  & 2026 & \textbf{0.921} & \textbf{0.894} & 19.3 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item En \textbf{MTL-AQA}, los \textit{Students} 2D quedan aproximadamente 7--8 puntos por debajo del SOTA (TPT, CoRe). X3D-M alcanza $0.8937$ \ac{SRCC}, superando al \textit{Teacher} \ac{I3D} ($0.8869$) pero aún por debajo de los métodos SOTA específicos para clavados (\textit{Temporal Parser}, regresión contrastiva, atención por grupos), que requieren \textit{backbones} 3D pesados. La brecha en MTL-AQA refleja una limitación estructural conocida: el dataset es altamente especializado y los métodos SOTA explotan componentes diseñados específicamente para esa tarea.
...
\paragraph{Ablación: reseteo de estadísticas y número de pases.} Para explorar el comportamiento heterogéneo de TTA-BN, se evaluaron dos hiperparámetros clave en dos pares representativos (el de mayor ganancia y el de mayor pérdida en la Tabla~\ref{tab:tta_crossdomain}): el número de pases de adaptación (1 vs 2) y la decisión de resetear las estadísticas del \textit{source} antes de adaptar (\textit{reset True/False}). Los resultados se reportan en la Tabla~\ref{tab:tta_ablation}.

\begin{table}[H]
\centering
\caption{Ablación de TTA-BN: efecto del número de pases y del reseteo de las estadísticas del \textit{source}. Reset=False mantiene las estadísticas del source y las mezcla con las del target.}
\label{tab:tta_ablation}
\small
\begin{tabular}{lllccc}
\toprule
\textbf{Source} & \textbf{Target} & \textbf{Arch} & \textbf{Pases} & \textbf{Reset} & \textbf{SRCC}↑ \\
\midrule
MTL-AQA & JIGSAWS & MBv3 & 1 & True  & \textbf{0.3288} \\
MTL-AQA & JIGSAWS & MBv3 & 2 & True  & 0.0454 \\
MTL-AQA & JIGSAWS & MBv3 & 1 & False & $-0.2468$ \\
MTL-AQA & JIGSAWS & MBv3 & 2 & False & $-0.1676$ \\
\midrule
MTL-AQA & AQA-7   & MBv3 & 1 & True  & $-0.1282$ \\
MTL-AQA & AQA-7   & MBv3 & 2 & True  & 0.4407          \\
MTL-AQA & AQA-7   & MBv3 & 1 & False & \textbf{0.4711} \\
MTL-AQA & AQA-7   & MBv3 & 2 & False & 0.4708          \\
\bottomrule
\end{tabular}
\end{table}

Adicionalmente, se evaluó TTA-BN sobre X3D-M en los seis pares cross-domain. A diferencia de los \textit{Students} 2D, el aporte de TTA-BN sobre X3D-M es marginal: sólo 2 de 6 configuraciones mejoran, con ganancia máxima de $+0.012$ \ac{SRCC} (y la pérdida máxima de $-0.143$). Una posible interpretación es que las arquitecturas 3D ya integran información temporal en sus estadísticas internas, lo que reduce la sensibilidad al \textit{covariate shift} que BN-recalibration corrige. Esto sugiere que TTA-BN es complementario y útil específicamente sobre arquitecturas 2D+TSM con muchas capas \textit{BatchNorm} espaciales por \textit{frame}, no como mejora universal.

La ablación revela también un patrón consistente: la configuración óptima de TTA-BN depende del grado de \textit{domain shift}. En MTL-AQA $\to$ JIGSAWS (deporte $\to$ cirugía, \textit{shift} severo) la mejor opción es \textit{reset=True} con un pase ($0.3288$); resetear las estadísticas del \textit{source} es necesario porque éstas no transfieren. En MTL-AQA $\to$ AQA-7 (deporte $\to$ deporte, \textit{shift} moderado) la mejor opción es \textit{reset=False} ($0.4711$), recuperando el comportamiento catastrófico ($-0.128$) que aparecía con \textit{reset=True}; mantener las estadísticas del \textit{source} y mezclarlas con las del \textit{target} preserva información útil compartida entre dominios cercanos. Este resultado motiva una versión \emph{selectiva} de TTA-BN que decida el modo de reseteo en función de una estimación del \textit{shift} (e.g., distancia entre estadísticas previas y nuevas); su formulación y evaluación quedan como dirección clara de trabajo futuro.
