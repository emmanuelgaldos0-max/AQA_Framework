...
\begin{itemize}
    ...
    \item \textbf{OE6 — Límites de aplicabilidad:} La transferencia cruzada MTL-AQA $\to$ AQA-7 obtiene \ac{SRCC} parcial ($\sim 0.55$); la transferencia AQA-7 $\to$ JIGSAWS falla (\ac{SRCC} cercano a cero). El análisis marginal de la destilación muestra que, sobre la línea base liviana fuerte, la \ac{KD} no aporta valor consistente: sólo 1 de 6 configuraciones (MobileNetV3 + AQA-7) mejora con \ac{KD}, mientras que las otras cinco degradan, con casos severos en JIGSAWS. Esto delimita el alcance del \textit{pipeline}: efectivo dentro del dominio entrenado, no universal.
\end{itemize}
...
\textbf{Tercero}, el análisis marginal de la destilación de conocimiento (\textbf{OE6}) muestra que, cuando el \textit{Student} se entrena con un \textit{pipeline} fuerte, la \ac{KD} no aporta valor consistente y puede incluso degradar el rendimiento. Esto cuestiona la premisa heredada de la literatura previa---según la cual el cierre de brecha entre \textit{Teacher} y \textit{Student} requiere destilación---y replantea la prioridad de futuras propuestas en \ac{AQA} eficiente.

En conjunto, la tesis entrega un \textit{pipeline} liviano reproducible, eficiente y empíricamente caracterizado para \ac{AQA} en dispositivos con recursos limitados, con sus componentes y sus límites de aplicabilidad claramente delimitados.
